        ...
        <div v-else class="users-grid">
          <div v-for="user in filteredUsers" :key="user.id" class="user-card">
            <div class="user-card-top">
              ...
              <div class="user-meta">
                <h3 class="user-nickname" :class="{ 'sponsor-nickname': user.is_sponsor }"><TeamBadge :teamId="user.team_id" />{{ user.nickname }}</h3>
            ...
            <div class="user-actions">
              ...
              <button
                @click="toggleSponsor(user.id)"
                class="action-btn sponsor"
                :class="{ 'sponsor-active': user.is_sponsor }"
                :disabled="isUpdating === user.id"
              >
                <svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z"/></svg>
                {{ user.is_sponsor ? 'Убрать спонсора' : 'Спонсор' }}
              </button>
              <button
                @click="toggleDeferredPayment(user.id)"
                class="action-btn deferred"
                :class="{ 'deferred-active': user.can_defer_payment }"
                :disabled="isUpdating === user.id"
              >
                <svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                {{ user.can_defer_payment ? '✓ Отложенная оплата' : 'Отложенная оплата' }}
              </button>
            </div>
          </div>
        </div>
...
async function toggleSponsor(userId: string) {
  const user = users.value.find(u => u.id === userId)
  if (!user) return
  ...
  try {
    ...
    if (error) {
      logger.error('Error toggling sponsor:', error)
      showToast('Ошибка изменения статуса спонсора: ' + error.message, 'error')
      return
    }

    user.is_sponsor = newValue
    showToast(
      newValue
        ? `${user.nickname} теперь спонсор ⭐`
        : `${user.nickname} больше не спонсор`,
      'success'
    )
  } catch (err: unknown) {
    logger.error('Error in toggleSponsor:', err)
    const message = err instanceof Error ? err.message : 'Неизвестная ошибка'
    showToast('Произошла ошибка: ' + message, 'error')
  } finally {
    isUpdating.value = null
...
async function toggleDeferredPayment(userId: string) {
  const user = users.value.find(u => u.id === userId)
  if (!user) return
  const newValue = !user.can_defer_payment
  isUpdating.value = userId
  try {
    const { error } = await supabase
      .from('users')
      .update({ can_defer_payment: newValue })
      .eq('id', userId)

    if (error) {
      logger.error('Error toggling deferred payment:', error)
      showToast('Ошибка изменения статуса отложенной оплаты: ' + error.message, 'error')
      return
    }

    user.can_defer_payment = newValue
    showToast(
      newValue
        ? `${user.nickname} может оплатить позже ⏰`
        : `${user.nickname} должен оплатить в срок`,
      'success'
    )
  } catch (err: unknown) {
    logger.error('Error in toggleDeferredPayment:', err)
    const message = err instanceof Error ? err.message : 'Неизвестная ошибка'
    showToast('Произошла ошибка: ' + message, 'error')
  } finally {
    isUpdating.value = null
...
.action-btn.sponsor:hover:not(:disabled) {
  background: rgba(251, 191, 36, 0.15);
}

.action-btn.sponsor.sponsor-active {
  background: rgba(251, 191, 36, 0.1);
  color: #f59e0b;
  border-color: rgba(251, 191, 36, 0.5);
}

.action-btn.sponsor.sponsor-active:hover:not(:disabled) {
  background: rgba(251, 191, 36, 0.2);
}
...
.action-btn.deferred.deferred-active {
  background: rgba(139, 92, 246, 0.1);
  color: #a78bfa;
  border-color: rgba(139, 92, 246, 0.5);
}

.action-btn.deferred.deferred-active:hover:not(:disabled) {
